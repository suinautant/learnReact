...
describe('<Input>', () => {
  ...
  describe('contains <input>', () => {
    ...
    it('assigns the prop value and type', () => {
      const epxpectedValue = '123';
      const wrapper = shallow(<Input name="test_name" value={epxpectedValue} />);
      expect(wrapper.find('input').prop('value')).toBe(epxpectedValue);
      const { type, value } = wrapper.find('input').props();
      expect(value).toBe(epxpectedValue);
